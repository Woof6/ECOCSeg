...
class DACS_ecoc(UDADecorator):
    ...
    def cal_score(self,seg_logit):
        ...
        top_k = 4
        similarity_k,pseudo_label_k =torch.topk(pred ,top_k,dim=1) #b,4,h,w
        bin_labels = self._expand_ecoc_multi_labels(pseudo_label_k, torch.tensor(self.code_book, dtype= torch.bool,device = seg_logit.device)) #b,4,k,h,w
        ...
        for i in range(top_k):
            ...
            pseudo_prob_k[:,i,:,:] = torch.sum(prob*mask_k[:,i,:,:,:],1)/torch.sum(mask_k[:,i,:,:,:],1)
            
      
        return  pseudo_prob_k[:,0,:,:],pseudo_label_k[:,0,:,:], similarity_k[:,0,:,:],similarity_k,pseudo_label_k,pseudo_prob_k,mask_k# b,h,w
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        pseudo_prob,pseudo_label ,similarity ,similarity_k,pseudo_label_k,pseudo_prob_k,mask_k=   self.cal_score(ema_logits.detach())          #b,h,w
        ...
        # Train on mixed images
        mix_losses = self.get_model().forward_train(
            mixed_img, img_metas, mixed_lbl, [pseudo_weight,pseudo_label_train], return_feat=True)
        mix_losses.pop('features')
        mix_losses = add_prefix(mix_losses, 'mix')
        mix_loss, mix_log_vars = self._parse_losses(mix_losses)
        log_vars.update(mix_log_vars)
